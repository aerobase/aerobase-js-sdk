== AeroBase JavaScript Security SDK

AeroBase JavaScript SDK offers a set of security features that will make sure that your application is meeting the modern OWASP recomendations. All security features are available as part of the separate package `@aerobase/security`. This package offers multiple security checks that can be used to analyse the security of the device. This can be useful for developers to detect vulnerabilities and react accordingly. For example, if the app detects security issue *X* then do *Y*, where *X* could be no lock screen is set and *Y* is show a system prompt to setup the lock screen.
...
4. Integration with the AeroBase Services Metrics SDK to report self-defence checks data.
...
+
----
npm install @aerobase/security
----

2. Add the AeroBase Security plugin for Cordova:
+
----
cordova plugin add cordova-plugin-aerobase-security
----
...
To begin using the self defence checks in your app, you'll need to import `SecurityService` from `@aerobase/security`:

[source,javascript]
----
import { SecurityService } from '@aerobase/security';
----
...
Custom self defence checks can be used with the Security SDK. Custom self defence checks must implement the link:https://github.com/aerobase/aerobase-js-sdk/blob/master/packages/security/src/deviceTrust/SecurityCheck.ts[SecurityCheck] interface.
...
The results of security checks can be published to the AeroBase metrics service.

This functionality requires the cordova plugin @aerobase/cordova-plugin-aerobase-security to be installed.
