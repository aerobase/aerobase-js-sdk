== Getting Started with the AeroBase JavaScript SDK
...
1. Get your app's configuration from the link:https://github.com/aerobase/mobile-cli[AeroBase Mobile CLI]. This JSON file contains custom confguration data for your project and shouldn't be changed manually. Every time a new service is provisioned the file has to be replaced by a new one.

2. Install the AeroBase SDK base project "app" in your Javascript project, which is available on NPM under the @aerobase org. Run the following command on your terminal:
+
[source.bash]
npm install @aerobase/app
+
...
You can now use any of the services that AeroBase provides, some of them require additional dependencies and steps:
